...
def test_parse_classify_response_valid():
    ...
    raw = json.dumps({
        "categories": [
            {"name": "filesystem", "description": "文件操作", "tools": ["read_file", "write_file"]},
            {"name": "calculation", "description": "计算", "tools": ["calculate"]},
        ]
    })
    result = parse_classify_response(raw)
    assert "tool_filesystem" in result
    assert result["tool_filesystem"]["tools"] == ["read_file", "write_file"]
    assert "tool_calculation" in result


def test_parse_classify_response_code_block():
    from src.tools.classifier import parse_classify_response
    raw = '```json\n{"categories": [{"name": "test", "description": "d", "tools": ["t1"]}]}\n```'
    result = parse_classify_response(raw)
...
def test_build_split_prompt():
    ...
    category = {"description": "文件操作", "tools": [f"t{i}" for i in range(10)]}
...
async def test_classify_tools_with_overflow_split():
    ...
    first_response = json.dumps({
        "categories": [
            {"name": "big_group", "description": "所有工具", "tools": [f"t{i}" for i in range(10)]},
        ]
    })
    split_response = json.dumps({
        "subcategories": [
            {"name": "group_a", "description": "A 组", "tools": [f"t{i}" for i in range(5)]},
            {"name": "group_b", "description": "B 组", "tools": [f"t{i}" for i in range(5, 10)]},
        ]
    })
    mock_llm = AsyncMock()
